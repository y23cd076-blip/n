...
# -------------------- AUTH (SUPABASE TABLE, USERNAME+PASSWORD) --------------------
def signup(username: str, password: str) -> str | None:
    username = (username or "").strip()
    if not username or not password:
        return "Username and password required"

    existing = sb_rest("GET", f"/rest/v1/{USER_TABLE}", params={"username": f"eq.{username}", "select": "id"})
    if existing:
...
def login(username: str, password: str) -> str | None:
    ...
    rows = sb_rest(
        "GET",
        f"/rest/v1/{USER_TABLE}",
        params={"username": f"eq.{username}", "select": "id,password_hash,username"},
    )
    if not rows:
...
def login_as_guest() -> None:
    """Create an ad-hoc guest user record so guest data is still stored in Supabase."""
    ...
    try:
        ...
        # Even if insert fails (e.g. RLS), still allow an in-memory guest session.
        pass
...
if not st.session_state.authenticated:
    ...
    with tab2:
        ...
        if st.button("Signup"):
            try:
                err = signup(new_username, new_password)
            except Exception as e:
                err = str(e)
            if err:
                st.error(err)
    ...
    with tab3:
        st.markdown("Use SlideSense without creating an account.")
...
if question:
    ...
    try:
        ...
        st.error("There was an error talking to the Gemini model. Please check your GOOGLE_API_KEY, project, and model name/configuration.")
        answer = "I couldn't generate an answer because of an error with the Gemini model configuration."
