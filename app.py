...
@st.cache_resource
def load_semantic_models() -> Tuple[Any, Any, Any]:
    try:
    ...
    world = World()
    _ = get_ontology("http://example.org/ontology#").load(world=world)
    return nlp, sbert, world
...
def _detect_intent_and_slot(text: str) -> Tuple[str, str]:
    t = text.lower()
    if "name" in t:
        return "ask_identity", "name"
    if "where" in t and "you" in t:
        return "ask_location", "location"
    return "unknown", "unknown"


def _extract_entities(doc) -> Dict[str, str]:
...
def _build_rdf_triples(intent: str, slot: str) -> Tuple[List[List[str]], Graph]:
    g = Graph()
    ex = Namespace("http://example.org/")
    g.bind("ex", ex)

    user = URIRef(ex.user)
    assistant = URIRef(ex.assistant)

    triples: List[List[str]] = []
    if intent == "ask_identity" and slot == "name":
        g.add((user, ex.asks, assistant))
        g.add((assistant, ex.property, Literal("name")))
        triples.append(["user", "asks", "assistant_name"])
    elif intent == "ask_location" and slot == "location":
        g.add((user, ex.asks, assistant))
        g.add((assistant, ex.property, Literal("location")))
        triples.append(["user", "asks", "assistant_location"])
    else:
        g.add((user, ex.utterance, Literal(slot)))
        triples.append(["user", "utters", slot])

    return triples, g


def semantic_intelligence_engine(user_input: str) -> Dict[str, Any]:
    ...
    corrected_sentence = _normalize_sentence(cleaned)

    nlp, sbert, _ = load_semantic_models()
    doc = nlp(corrected_sentence) if nlp is not None else None

    entities = _extract_entities(doc) if doc is not None else {}

    intent, slot = _detect_intent_and_slot(corrected_sentence)

    if intent == "ask_identity":
        semantic_meaning = "user is requesting the assistant's name"
    elif intent == "ask_location":
        semantic_meaning = "user is requesting location information"
    else:
        semantic_meaning = "user utterance with no mapped intent"

    rdf_triples, _ = _build_rdf_triples(intent, slot)
...
def render_login_ui() -> None:
    ...
    with col1:
        # Login page animation (kept as you requested)
        try:
            anim = requests.get(
            ...
            if anim.status_code == 200:
                st_lottie(anim.json(), height=260)
        except Exception:
            pass
    ...
    if is_guest:
        ...
        if "guest_db_user" not in st.session_state:
            ...
            if rows:
                ...
                # Try to create a guest user; if it fails (e.g. tables not created yet),
                # fall back to an in-memory guest identity so the app still works.
                try:
                    created = sb_post(
                        f"/rest/v1/{USER_TABLE}",
                        json_body={
                    ...
                    st.session_state["guest_db_user"] = {
                        "id": "guest-local",
                        "username": "guest",
                    }
