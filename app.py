...
def change_password(old_password: str, new_password: str) -> Optional[str]:
    ...
    resp = _rest_request(
        "PATCH",
        f"/rest/v1/{USER_PROFILES_TABLE}?id=eq.{row['id']}",
        json={"password_hash": _hash_password(new_password)},
    )
    if resp.status_code >= 400:
        return resp.text
    return None
...
if st.session_state.chat_history:
    ...
    items = list(reversed(list(enumerate(st.session_state.chat_history, start=0))))
    labels = [f"{idx + 1}. {q[:40]}..." for idx, (_, (q, _)) in enumerate(items)]
    ...
    if selected_label:
        sel_display_idx = int(selected_label.split(".")[0]) - 1
        original_idx = items[sel_display_idx][0]
        q_sel, a_sel = st.session_state.chat_history[original_idx]
...
if mode == "📘 PDF Analyzer":
    ...
    if pdf:
        ...
        # Show selected history (if any) at top of main area
        selected_hist = st.session_state.get("selected_history")
        if selected_hist:
            st.markdown("### 🔁 Selected from history")
            with st.chat_message("user"):
                st.markdown(selected_hist["question"])
            with st.chat_message("assistant"):
                render_answer_with_copy(
                    selected_hist["answer"], key_suffix="history_selected"
                )
