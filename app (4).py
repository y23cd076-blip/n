...
def login_ui() -> None:
    ...
    # ── Login page CSS + canvas animation (inherits CSS vars, works in light & dark) ──
    st.markdown("""
    <style>
    ...
    .auth-logo {
        ...
        margin-bottom: .7rem;
        animation: auth-logo-in .6s cubic-bezier(.34,1.56,.64,1) both;
    }
    ...
    .auth-card {
        ...
        box-shadow: var(--shadow-card);
        border: 1px solid var(--border);
        animation: rise .55s cubic-bezier(.22,1,.36,1) .1s both;
        position: relative; overflow: hidden;
    }
    /* Subtle sheen overlay on card */
    .auth-card::before {
        content: '';
    ...
    .auth-toggle {
        ...
        animation: fade-in .5s ease .3s both;
    }
    .auth-toggle-btn {
        flex: 1; border: none; cursor: pointer;
        border-radius: 999px; padding: .42rem 0;
        font-size: .84rem; font-weight: 600;
        font-family: 'DM Sans', sans-serif;
        background: transparent;
        color: var(--text-muted);
        transition: background .25s, color .25s, transform .2s cubic-bezier(.34,1.56,.64,1), box-shadow .25s;
    }
    ...
    .auth-form-title {
        ...
        margin-bottom: .18rem;
        animation: slide-x .38s cubic-bezier(.22,1,.36,1) both;
    }
    .auth-form-sub {
        font-size: .81rem; color: var(--text-muted);
        margin-bottom: 1.1rem;
        animation: slide-x .38s cubic-bezier(.22,1,.36,1) .06s both;
    }

    /* Divider */
    .auth-divider {
        display: flex; align-items: center; gap: .65rem;
        margin: .9rem 0; color: var(--text-faint); font-size: .77rem;
    }
    ...
    /* Field stagger */
    .stTextInput:nth-of-type(1) { animation: rise .32s ease .18s both; }
    .stTextInput:nth-of-type(2) { animation: rise .32s ease .26s both; }
    .stTextInput:nth-of-type(3) { animation: rise .32s ease .34s both; }
    </style>
    ...
    (function(){
        ...
        /* Detect current theme colour for particles */
        function accentHue() {
        ...
        function draw() {
            ...
            pts.forEach(p => {
                ...
                ctx.fillStyle = `hsla(${h},68%,68%,${p.o})`;
                ctx.fill();
                p.x += p.vx; p.y += p.vy; p.o -= .0012;
                if (p.y < -8 || p.o <= 0) {
                    p.x = Math.random() * c.width;
                    p.y = c.height + 8;
                    p.o = Math.random() * .45 + .2;
                    p.r = Math.random() * 2.2 + .6;
                }
            });
            ...
            for (let i = stars.length - 1; i >= 0; i--) {
                const s = stars[i];
                const grad = ctx.createLinearGradient(s.x, s.y, s.x - s.len, s.y - s.len/2);
                grad.addColorStop(0, `hsla(${h},80%,75%,${s.o})`);
                grad.addColorStop(1, `hsla(${h},80%,75%,0)`);
                ctx.beginPath();
                ctx.moveTo(s.x, s.y);
                ...
                ctx.lineWidth = 1.4;
                ctx.stroke();
                s.x += s.vx; s.y += s.vy; s.o -= .014;
                if (s.o <= 0) stars.splice(i, 1);
            }
    ...
    <div class="auth-wrap">
      ...
      <div class="auth-brand-name">SlideSense</div>
      <div class="auth-tagline">Smart Learning · Smart Vision · Smart AI</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    # 3-tab pill toggle (visual only)
    active_login  = "active" if mode == "login"  else ""
    active_signup = "active" if mode == "signup" else ""
    active_guest  = "active" if mode == "guest"  else ""
    st.markdown(f"""
    <div class="auth-toggle">
      <button class="auth-toggle-btn {active_login}"  disabled>Login</button>
      <button class="auth-toggle-btn {active_signup}" disabled>Sign Up</button>
      <button class="auth-toggle-btn {active_guest}"  disabled>Guest Mode</button>
    </div>""", unsafe_allow_html=True)

    # Functional toggle buttons (drive state)
    ...
    with c3:
        if st.button("Guest Mode", key="toggle_guest"):
            st.session_state["auth_mode"] = "guest"; st.rerun()

    st.markdown("<hr style='border:none;border-top:1px solid #f0ebfa;margin:.5rem 0 1rem'>",
                unsafe_allow_html=True)
    ...
    # ── GUEST MODE TAB ─────────────────────────────────────────────────────────
        ...
        st.markdown('<div class="auth-form-title">Guest Mode 👤</div>', unsafe_allow_html=True)
        st.markdown(
            "<div class='auth-form-sub'>Try SlideSense without an account.<br>"
            "Chat history won't be saved between sessions.</div>",
            unsafe_allow_html=True,
        )
